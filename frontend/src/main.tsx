...
function App() {
  ...
  const [names, setNames] = useState<Record<string, string>>({ speaker_0: 'Alex', speaker_1: 'Jamie', speaker_2: 'Sam' });
  ...
  const busy = state.status === 'connecting' || state.status === 'processing';
  const speaker = (id: string | null) => id === null ? 'Unknown speaker' : names[id] || id.replace('speaker_', 'Speaker ');
  const emit = (event: MeetingEvent) => setState(s => reduceEvent(s, event));
  ...
  function start() { session.current?.dispose(); setState(initialState); setSeconds(0); setSelected([]); followTranscript.current = true; setNames(n => ({ speaker_0: n.speaker_0 || 'Alex', speaker_1: n.speaker_1 || 'Jamie', speaker_2: n.speaker_2 || 'Sam' })); setImported(false); session.current = startSession(emit); }
  function stop() { session.current?.stop(); }
  ...
  function download() {
    const blob = new Blob([JSON.stringify({ ...state, speakers: names, transcript_version: state.transcript.length, based_on_transcript_version: state.analysis.based_on_transcript_version, analysis_is_current: state.analysis.based_on_transcript_version === state.transcript.length, mock_conversation: !imported, analysis_source: imported ? 'saved' : 'live_agent_loop' }, null, 2)], { type: 'application/json' });
  ...
  return <div className="app">
    ...
    <div className="workspace">
      ...
      <main>
        ...
        <div className="demo-notice"><span><b>{imported ? 'Imported meeting' : 'Mock conversation · Live agent'}</b> · {imported ? 'Saved Agent Loop output. No live microphone.' : 'Speech is simulated. Summaries and highlights come from DeepSeek; searches run through Exa.'}</span><button onClick={() => file.current?.click()} disabled={running}>Load agent export <span>↗</span></button><input ref={file} type="file" accept=".json,application/json" hidden onChange={importMeeting}/></div>
        ...
        <div className="board">
          <section className="panel transcript-panel"><div className="panel-heading"><span className="section-icon peach"><Icon name="mic"/></span><div><h2>The conversation</h2><p>Every voice, in its own color</p></div><span className="count">{state.transcript.length.toString().padStart(2, '0')}</span></div><div ref={transcriptList} onScroll={e => { const list = e.currentTarget; followTranscript.current = list.scrollHeight - list.scrollTop - list.clientHeight < 40; }} className="transcript-list" aria-label="Meeting transcript">
            {!state.transcript.length && !state.partial && <div className="empty"><div className="wave">▂ ▅ ▃ ▇ ▄ ▂ ▆ ▃ ▅ ▂</div><h3>It starts with a conversation.</h3><p>Hit start. Each speaker’s words will find a place here.</p><span>MICROPHONE NOT CONNECTED</span></div>}
          ...
          </div><div className="panel-footer"><i className={running ? 'pulse' : ''}/>{state.status === 'recording' ? 'Sending sample speech to Agent Loop' : 'Mock speech · Deepgram not connected'}<span>{state.transcript.length} entries</span></div></section>
        ...
        </div><footer><span className="wordmark">parley<span>✳</span></span><span>Less note-taking. More being here.</span><span>{imported ? 'SAVED MEETING' : 'REAL AGENT / MOCK SPEECH'}</span></footer>
      </main>
    </div>
  </div>;
}
